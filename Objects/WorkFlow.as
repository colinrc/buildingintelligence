		...
		*/
		public function addNode(key:String, inst:MyTreeNode):void {
			...
			if (serverHash.containsKey(lKey) == true) {
				...
				order = 0;
				found = false;
				var tempXML:XML = clientHash.getValue(lKey);
				var tempNode:MyListNode = new MyListNode(tempXML, inst);
				order = parseInt(tempXML..@order);
				if (clientList.length == 0) {
					clientList.addItemAt({ label: tempXML.@label.toString(), key: key, description: tempXML.@description.toString(), icon: valid, order: parseInt(tempXML.@order.toString()), obj: inst.object },0);
				}
				else {
					for (var i:int= 0 ; i < clientList.length ; i++) {
						index = clientList.getItemAt(i).order;
						if (index > order) {
							clientList.addItemAt({ label: tempXML.@label.toString(), key: key, description: tempXML.@description.toString(), icon: valid, order: parseInt(tempXML.@order.toString()), obj: inst.object },i);
							found = true;
							break;
						}
					}
					if (found == false) {
						clientList.addItemAt({ label: tempXML.@label.toString(), key: key, description: tempXML.@description.toString(), icon: valid, order: parseInt(tempXML.@order.toString()), obj: inst.object },clientList.length);
					}
				}
