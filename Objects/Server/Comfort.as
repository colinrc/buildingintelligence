package Objects.Server {
	...
	public class Comfort extends Device {
		...
		public function newObject():void {
			...
			pulse_outputs = new Toggles("PULSE_OUTPUT");
			toggle_outputs = new Toggles("TOGGLE_OUTPUT");
			toggle_inputs = new Toggles("TOGGLE_INPUT");
		...
		public override function setXML(newData:XML):void {
			...
			if (newData.name() == "DEVICE") {
				...
				for (var child:int=0; child < newData.length();child++) {
					...
					case "COMFORT" :
						...
						for (var rawDevice:int ; rawDevice < tempNode.length() ; rawDevice++) {
