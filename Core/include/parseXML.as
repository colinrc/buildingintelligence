...
defineAppsBar = function (icons) {
	for (var icon=0; icon<icons.length; icon++) {
		_global.appsBar.push({name:icons[icon].attributes.name, icon:icons[icon].attributes.icon, func:icons[icon].attributes.func, program:icons[icon].attributes.program, canOpen:icons[icon].attributes.canOpen});
	}
}
...
defineZones = function (zones) {
	for (var zone=0; zone<zones.length; zone++) {
		_global.zones.push({name:zones[zone].attributes.name, map:zones[zone].attributes.map, cycle:(zones[zone].attributes.cycle=="true"), alignment:zones[zone].attributes.alignment, canOpen:zones[zone].attributes.canOpen, rooms:[], panels:[], arbitrary:[]})
		for (var z=0; z<zones[zone].childNodes.length; z++) {
			if (zones[zone].childNodes[z].nodeName == "rooms") {
				...
				for (var room=0; room<rooms.length; room++) {
					_global.zones[zone].rooms.push({name:rooms[room].attributes.name,poly:rooms[room].attributes.poly, canOpen:rooms[room].attributes.canOpen, groups:[],alerts:[], doors:[], alertsPos:{}})
					for (var i=0; i<rooms[room].childNodes.length; i++) {
						switch (rooms[room].childNodes[i].nodeName) {
							case ("controls") :
								var groups = rooms[room].childNodes[i].childNodes;
								for (var group=0; group<groups.length; group++) {
									_global.zones[zone].rooms[room].groups.push({name:groups[group].attributes.name, icon:groups[group].attributes.icon, canSee:groups[group].attributes.canSee, controls:[]});
									var controls = groups[group].childNodes;
									for (var control=0; control<controls.length; control++) {
										if (controls[control].attributes.key != undefined && _global.controls[controls[control].attributes.key] == undefined) _global.controls[controls[control].attributes.key] = {key:controls[control].attributes.key, zone:zones[zone].attributes.name, room:rooms[room].attributes.name, name:controls[control].attributes.name, storedStates:new Object()};
										//_global.controls[controls[control].attributes.key].storedStates["state"] = "on";
										_global.zones[zone].rooms[room].groups[group].controls[control] = new Object();
										for (var attrib in controls[control].attributes) {
											_global.zones[zone].rooms[room].groups[group].controls[control][attrib] = controls[control].attributes[attrib];
										}
									}
								}
								break;
							case ("alerts") :
								...
								_global.zones[zone].rooms[room].alertsPos = {x:alert.attributes.x, y:alert.attributes.y, layout:alert.attributes.layout};
								var alerts = alert.childNodes;
								for (var alert=0; alert<alerts.length; alert++) {
									_global.zones[zone].rooms[room].alerts.push({name:alerts[alert].attributes.name,keys:alerts[alert].attributes.keys, icon:alerts[alert].attributes.icon, fadeOutTime:alerts[alert].attributes.fadeOutTime});
...
defineCalendarData = function (events) {
	...
	for (var event=0; event<events.length; event++) {
		...
		if (d.eventType == "once") {
			_global.calendarData.push({title:d.title, memo:d.description, startDate:d.date.parseDate(), endDate:d.date.parseDate(), time:d.time, eventType:"once", macroName:d.macroName});
		} else {
			var pattern = new Object();
			for (var attrib in events[event].firstChild.attributes) {
				pattern[attrib] = events[event].firstChild.attributes[attrib];
			}
			_global.calendarData.push({title:d.title, memo:d.description, startDate:d.startDate.parseDate(), endDate:d.endDate.parseDate(), time:d.time, eventType:d.eventType, macroName:d.macroName, pattern:pattern});
		}
	}
}
...
defineControlPanelApps = function (apps) {
	for (var app=0; app<apps.length; app++) {
		_global.controlPanelApps.push({label:apps[app].attributes.label, program:apps[app].attributes.program});
	}
}
