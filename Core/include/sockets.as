...
serverOnConnect = function (status) {
	...
	if (status) {
		...
		if (_global.settings.debugMode == "false") {
			showCommsError();
			commsError = true;
		}
...
serverConnect = function () {
	debug("Connecting to server: " + _global.settings.serverAddress + ":" + _global.settings.serverPort);
	if (_global.settings.serverAddress.length) server.connect(_global.settings.serverAddress, _global.settings.serverPort)
}
