...
class Forms.Project.Client.ArbitraryObject extends Forms.BaseForm {
	...
	public function onLoad():Void {
		...
		for (var tempIcon = 0; tempIcon < show_cmb.dataProvider.length; tempIcon++) {
			if (show_cmb.dataProvider[tempIcon].label == currentAlert.show) {
				show_cmb.selectedIndex = tempIcon;
				break;
			}
		}
		hide_cmb.dataProvider = DPControl;
		for (var tempIcon = 0; tempIcon < hide_cmb.dataProvider.length; tempIcon++) {
			if (hide_cmb.dataProvider[tempIcon].label == currentAlert.hide) {
				hide_cmb.selectedIndex = tempIcon;
				break;
			}
		}
