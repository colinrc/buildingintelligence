class Objects.Server.SignVideo extends Objects.Server.Device {
	...
	public function toXML():XMLNode {
		...
		newParameter.attributes["VALUE"] = "Sign Video Inputs";
		...
		var newSignVideo = new XMLNode(1,device_type);
		var tempAudioVideos = audiovideos.toXML();
		for (var child in tempAudioVideos.childNodes){
			newSignVideo.appendChild(tempAudioVideos.childNodes[child]);
		}
	...
	public function setXML(newData:XMLNode):Void {
		...
		newInputs.attributes["NAME"] = "Sign Video Inputs";
			...
			for (var child in newData.childNodes) {
				...
				case "PARAMETERS" :
					for(var parameter in newData.childNodes[child].childNodes){
						if((newData.childNodes[child].childNodes[parameter].attributes["NAME"] != "AV_INPUTS")){						
							parameters.push(newData.childNodes[child].childNodes[parameter]);
						}
					}
					break;
				case "CATALOGUE" :
					if(newData.childNodes[child].attributes["NAME"] == "Sign Video Inputs"){
