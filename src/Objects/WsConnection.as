package Objects {
	...
	public class WsConnection {
		...
		public function WsConnection(server:Boolean):void {
			//todo: change to use web services for mdm.Application.path.substring(0, mdm.Application.path.length - 1);
			appPath = "";
			localPath = appPath + "/";
			isServer = server;		
			if(server){
			...
			view = undefined;
			lastCode = "0";
			/*var MyDll = new mdm.DLL("wodWS.dll");
			MyDll.addParameter("string","80");
			MyDll.call("none","wodWSComLib.wodWSCom.About");
			MyDll.close();*/
			//myActiveX = new mdm.ActiveX(0, 0, 0, 0, "BIwodWS.BiWS");
			//myActiveX.setProperty("LocalPath", "string", localPath);		
			//myActiveX.setProperty("RemotePath", "string", remotePath);
		}
		public function setLocalPath(inLocalPath:String):void {
			localPath = inLocalPath;
			//myActiveX.setProperty("LocalPath", "string", inLocalPath);
		}
		public function setRemotePath(inRemotePath:String):void {
			if (lastCode != "0") {
		...
		public function getRemoteList():Array {
			...
			if (lastError != "0") {
				...
				var dirList = myActiveX.getProperty("ListItem").split(String.fromCharCode(13) + String.fromCharCode(10));
		...
		public function connect(user:String, port:Number, hostname:String, password:String):void {
			...
			//intervalId = setInterval(checkStatus, duration,);
		}
		...
		public function processError(inCode:String):String {
			...
			case "30015" :
				...
				break;
			case "30016" :
				return "WS subsystem on server unavailable.";
				break;
			case "30017" :
		...
		public function connectionStatus(inCode:String):String {
			...
			return "Do not understand this code";
		}
	}
}