package Objects.Server {
	...
	public class Comfort extends Device {
		...
		[Bindable]
		public var raw_interfaces:Raw_Interfaces;
		[Bindable]
		public var toggle_monitors:Monitors;
		[Bindable]
		public var cbus_lights:CBusLights;
		[Bindable]
		public var cbus_relays:CBusRelays;			
		[Bindable]
		public var x10_lights:X10Lights;
		[Bindable]
		public var pulse_outputs:Toggles;
		[Bindable]
		public var toggle_inputs:Toggles;
		[Bindable]
		public var toggle_outputs:Toggles;
		[Bindable]
		public var alarms:Alarms;
		[Bindable]
		public var alerts:Alerts;
		[Bindable]
		public var analogues:Analogues;
		[Bindable]
		public var door_ids:Catalogue;
		[Bindable]
		public var comfort_users:Catalogue;
		[Bindable]
		...
		public override function get Data():ObjectProxy{
			var ob:ObjectProxy = new ObjectProxy({device_type:device_type, description:description, active:active, connection:connection, parameters:parameters, password:password, dataObject:this});
			return ob;
		}
		...
		public override function toXML():XML {
			...
			for (var child in tempCatalogues.children()) {
				newDevice.appendChild(tempCatalogues.children()[child]);
			}
			var newComfort = new XML("<"+device_type+" />");
			var tempCustoms = customs.toXML();
			for(var child in tempCustoms.children()){
				newComfort.appendChild(tempCustoms.children()[child]);
			}
			var tempRaw_Interfaces = raw_interfaces.toXML();
			for(var child in tempRaw_Interfaces.children()){
				newComfort.appendChild(tempRaw_Interfaces.children()[child]);
			}
			var tempCounters = counters.toXML();
			for(var child in tempCounters.children()){
				newComfort.appendChild(tempCounters.children()[child]);
			}
			var tempMonitors = toggle_monitors.toXML();
			for(var child in tempMonitors.children()){
				newComfort.appendChild(tempMonitors.children()[child]);
			}
			var tempCbusLights = cbus_lights.toXML();
			for(var child in tempCbusLights.children()){
				newComfort.appendChild(tempCbusLights.children()[child]);
			}
			var tempCbusRelays = cbus_relays.toXML();
			for(var child in tempCbusRelays.children()){
				newComfort.appendChild(tempCbusRelays.children()[child]);
			}		
			var tempX10Lights = x10_lights.toXML();
			for(var child in tempX10Lights.children()){
				newComfort.appendChild(tempX10Lights.children()[child]);
			}
			var tempPulseOutputs = pulse_outputs.toXML();
			for(var child in tempPulseOutputs.children()){
				newComfort.appendChild(tempPulseOutputs.children()[child]);
			}
			var tempToggleOutputs = toggle_outputs.toXML();
			for(var child in tempToggleOutputs.children()){
				newComfort.appendChild(tempToggleOutputs.children()[child]);
			}
			var tempToggleInputs = toggle_inputs.toXML();
			for(var child in tempToggleInputs.children()){
				newComfort.appendChild(tempToggleInputs.children()[child]);
			}
			var tempAlerts = alerts.toXML();
			for (var child in tempAlerts.children()){
				newComfort.appendChild(tempAlerts.children()[child]);
			}
			var tempAlarms = alarms.toXML();
			for (var child in tempAlarms.children()){
				newComfort.appendChild(tempAlarms.children()[child]);
			}
			var tempAnalogues = analogues.toXML();
			for (var child in tempAnalogues.children()){
				newComfort.appendChild(tempAnalogues.children()[child]);
			}
			var tempKeypad = keypad.toXML();
			for (var child in tempKeypad.children()){
				newComfort.appendChild(tempKeypad.children()[child]);
			}
		...
		public override function newObject():void {
			...
			raw_interfaces.raw_interfaces = new ArrayCollection();
			customs = new Customs();
				...
				for (var child:int=0; child < newData.children().length();child++) {
					...
					case "COMFORT" :
						...
						for (var rawDevice:int ; rawDevice < tempNode.children().length() ; rawDevice++) {
							var t1Name:String = tempNode.children()[rawDevice].name();
							switch (t1Name) {
							case "CUSTOM_INPUT" :
