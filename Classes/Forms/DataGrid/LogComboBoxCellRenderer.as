...
class Forms.DataGrid.LogComboBoxCellRenderer extends UIComponent {
	var label:MovieClip;
	...
	// the function we receive from the list
	function LogComboBoxCellRenderer() {
	}
		...
		if (item != undefined) {
			...
			if (itemObject.sel) {
				if (instanceName != "ComboBox") {
					...
					label.callBack = itemObject.callBack;
	...
	function change() {
		...
		label.callBack(listOwner.dataProvider[itemLocation.itemIndex].packagename,label.selectedItem.label);		
	}
}
